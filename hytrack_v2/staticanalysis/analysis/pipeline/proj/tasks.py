...
SOFT_TIME_LIMIT_SEC = 60 * 30

logger = get_task_logger(__name__)


@app.task(soft_time_limit=SOFT_TIME_LIMIT_SEC)
def analyze_apk(package_name, split_files):
    try:
        ...
        try:
            ...
            logger.error("Androguard failed for package {}".format(package_name))
            result['error'] = f'ANDROGUARD'
        ...
        # result["ctt_trusted_strings"] = find_trusted_strings(dx)  # fuzzy twa search
        result["additional_origin_calls"] = find_additional_trusted_origins_calls(dx)
        result["additional_origin_intent"] = find_additional_trusted_origins_intent(dx)
        result["additional_origin_manifest"] = find_additional_trusted_origins_manifest(a)
        result["twa_usage"] = is_twa_used(a, dx)
        result["dals"] = find_dal(a, dx)
        result["schemes"] = find_schemes(a)
...
def find_trusted_strings(dx):
    return [str(x.value) for x in dx.find_strings("(?i).*trusted.*")]


def is_twa_used(a, dx):
    result = {}
    result["INTENT_EXTRA"] = []
    # 1. check for TWA launcher activity
    ## https://developer.chrome.com/docs/android/trusted-web-activity/android-browser-helper-migration
    result["ACTIVITY_NEW"] = "com.google.androidbrowserhelper.trusted.LauncherActivity" in [a for a in
                                                                                            a.get_activities()]
    result["ACTIVITY_OLD"] = "android.support.customtabs.trusted.LauncherActivity" in [a for a in a.get_activities()]

    # 2. check for helper library calls
    # 2.1 TwaLauncher.launch()
    twalauncher_launch_calls = []
    for ma in dx.find_methods(classname="Lcom/google/androidbrowserhelper/trusted/TwaLauncher", methodname="launch"):
        for cls, method, _ in ma.get_xref_from():
            if cls.name.startswith("Lcom/google/androidbrowserhelper/"):
                # skip the calls in lib
                continue
            twalauncher_launch_calls.append(cls.name)
    result["LIB"] = {}
    result["LIB"]["TWALAUNCHER_LAUNCH"] = twalauncher_launch_calls

    #  2.2 TrustedWebUtils.launchAsTrustedWebActivity()
    deprecated_launches = []
    for m in dx.find_methods(classname="Landroidx/browser/customtabs/TrustedWebUtils",
                             methodname="launchAsTrustedWebActivity"):
        for cls, method, _ in m.get_xref_from():
            if cls.name.startswith("Landroidx/browser"):
                continue
            deprecated_launches.append(cls.name)
    result["LIB"]["TWU_LAUNCHASTWA"] = deprecated_launches
    # 3. check for intent extra flag outside of libraries.
    for stringanalysis in dx.find_strings("android.support.customtabs.extra.LAUNCH_AS_TRUSTED_WEB_ACTIVITY"):
        for classanalysis, methodanalyis in stringanalysis.get_xref_from():
            if classanalysis.name.startswith("Lcom/google/androidbrowserhelper/"):
                continue
            if methodanalyis.class_name.startswith("Landroidx/browser"):
                continue
            # it's not in a lib
            result["INTENT_EXTRA"].append(methodanalyis.class_name)
    return result


def find_additional_trusted_origins_calls(dx):
    addto = dx.find_methods(methodname="setAdditionalTrustedOrigins")
    ...
    for m in addto:
        ...
        result[str(m.method.full_name)] = callees

    return result


def find_additional_trusted_origins_manifest(a):
    return "android.support.customtabs.trusted.ADDITIONAL_TRUSTED_ORIGINS" in a.get_android_manifest_axml().get_xml().decode()


def find_additional_trusted_origins_intent(dx):
    result = []
    for stringanalysis in dx.find_strings("android.support.customtabs.extra.ADDITIONAL_TRUSTED_ORIGINS"):
        for classanalysis, methodanalyis in stringanalysis.get_xref_from():
            if classanalysis.name.startswith("Lcom/google/androidbrowserhelper/"):
                continue
            if methodanalyis.class_name.startswith("Landroidx/browser"):
                continue
            # it's not in a lib
            result.append(methodanalyis.class_name)
    return result
...
def find_dal(a, dx):
    result = {}
    result[
        "DAL_DIRECTLY_IN_MANIFEST"] = "delegate_permission/common.handle_all_urls" in a.get_android_manifest_axml().get_xml().decode()
    result["DAL_IN_DX_STRING"] = len(list(dx.find_strings(".*delegate_permission/common.handle_all_urls.*"))) > 0
    # now search in string resources
    relation_candidates: List[str] = []
    res = a.get_android_resources().get_resolved_strings()
    for namespace in res:
        for lang in res[namespace]:
            for key in res[namespace][lang]:
                string = res[namespace][lang][key]
                if not string:
                    continue
                if "delegate_permission/common.handle_all_urls" in string:
                    relation_candidates.append(string)
    relation_candidates = list(set(relation_candidates))

    relations = []
    for relation in relation_candidates:
        try:
            rel = json.loads(relation)
            relations.append(rel)
        except json.JSONDecodeError:
            pass
    result["DALS_FOUND_IN_STRING_RESOURCES"] = relations
    return result


def find_schemes(a) -> List[str]:
    activities = a.get_activities()
    all_schemes = set()
    for activity in activities:
        fltr = a.get_intent_filters('activity', activity)
        if "action" not in fltr or "category" not in fltr:
            continue
        if "android.intent.action.VIEW" not in fltr["action"]:
            continue
        if "android.intent.category.BROWSABLE" not in fltr["category"]:
            # it must be BROWSABLE to be launched
            continue
        if not fltr["data"]:
            continue
        schemes = [x["scheme"] for x in fltr["data"] if "scheme" in x]
...
def find_intent_filter(a):
    ...
    action_intents = manifest_tree.xpath(
        "/manifest/queries/intent/action[@*='android.support.customtabs.action.CustomTabsService']")
...
def find_builder_functions(dx):
    ...
    try:
        ...
        result = {}
        for f in functions:
            methods = dx.find_methods("Landroidx/browser/customtabs/CustomTabsIntent\$Builder",
                                      f)  # we need to escape the dollar sign, since this is a regex
            callees = []
            for m in methods:
                xrefs = m.get_xref_from()
                for _, meth, _ in xrefs:
...
def write_result_to_file(package_name, result):
    try:
        ...
        logger.error("An error occurred while writing result: {}".format(e))
        return {}
