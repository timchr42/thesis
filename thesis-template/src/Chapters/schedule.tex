...
\begin{itemize}
  ...
  \item \textbf{Phase 1 (Week 1):} \textit{Background Research and Literature Review} \\
  ...
  \item \textbf{Phase 2 (Weeks 2-4):} \textit{Framework Design and Initial Implementation} \\
  Design the capability-based access control framework.
  Define requirements, threat model, and integration points. \info{similar threat model to HyTrack (not consider malicious dev and thus false policies)}
  Begin implementation of the framework, touching on the Installer, App and Chromium browser components.

  \item \textbf{Phase 4 (Weeks 5):} \textit{Testing} \\
    Thoroughly test the framework to ensure it works as intended and meets the requirements. \change{write requirements here again?}

  \item \textbf{Phase 3 (Week 6):} \textit{Evaluation and Experimentation} \\
...
In case we encounter problems in adapting the Installer to send the policy to the browser, we will explore sending the policy on each app launch via a modified Zygote instead.
