...
We first describe the experimental setup (\autoref{sec:eval-setup}), before detailing the results (\autoref{sec:eval-results}) that we observed and finally interpreting our findings (\autoref{sec:interpret-results}).
...
Therefore, we removed the Android Browser Helper dependency and established the required session connection to Firefox manually, allowing the apps to open TWAs (or CT fallbacks) in our modified Fenix browser.
...
This app displays all capability tokens received from the browser when launching a CT or TWA to a specific domain, along with the corresponding cookies they encapsulate.
