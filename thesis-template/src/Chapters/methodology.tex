...
This policy -- declared by the app developer in the manifest and config file -- defines the following:
 ...
 According to this policy, the browser creates capabilities for the app wrapping important cookie metadata (e.g., global jar flag, domain, name and value) into a secure structure, encoded and signed by the browser, similar to JSON Web Tokens (JWT)~\cite{rfc7519}.
...
\begin{itemize}
  ...
  \item If it is new and no wildcard capability exists, the browser creates a new capability with the name and value, domain and app ID, restricted rights and version number, and returns it to the app for storage in its app-specific cookie jar.
\end{itemize}
