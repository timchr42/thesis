...
For instance, HyTrack demonstrated that a single shared third-party library used across multiple apps is sufficient to persistently identify and track users, bypassing conventional browser and OS sandboxing mechanism.
...
To mitigate the issue, we propose a developer-defined policy mechanism, combined with cryptographically enforced capabilities, to control how cookies are managed and isolated on a per-app basis.

% Developer Policy Declaration
During app installation, the installer extracts, validates, encrypts and transmits the policy to the browser.
This policy -- declared by the app developer in the manifest and config file -- defines the following:
 \begin{itemize}
   ...
   \item Optionally, a set of predefined cookie names expected to be used by the respectice servers.
 \end{itemize}
...
\item \textbf{Manifest} and \textbf{config file} for the developers to define which origins should retain shared browser state.
  \item A \textbf{Custom Installer} to extract, validate, encrypt and send the developer-defined policy to the browser upon app installation.
