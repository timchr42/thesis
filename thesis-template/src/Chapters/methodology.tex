...
\begin{itemize}
  ...
  \item \textbf{Predefined Capabilities:} Define fixed cookie names as specified in the policy and can be stored either in the app-specific cookie jar or the browser's shared cookie jar, depending on the configuration of the global jar flag.
    ...
    This provided fine-grained control over cookie scoping and allows the developers to isolate their app from the browser's state when desired, while still enabling seamless web-app integration.
...
\begin{itemize}
  ...
  \item If it's new and no wildcard capability exists, the browser creates a new capability with the name and value, domain and app ID, restricted rights and version number, and hands it back to the app to store it in its app-specific cookie jar.
\end{itemize}

\section{Benefits} 
Beyond eliminating cross-app tracking through HyTrack and their postulated goals, we identify the following additional benefits offered by this approach:
