...
% DUPLICATE from above therefore commented out
...
%The proposed framework prevents HyTrack's cross-app tracking attack \cite{USENIX:Wessels:2025} by eliminating the architectural weaknesses identified in \autoref{sec:hytrack-weaknesses}, while adhering to the design goals of usability, compatibility, and controlled access to shared state (see \autoref{sec:hytrack-goals}).

\paragraph{Core Idea.} The core idea is to encapsulate cookie access within fine-grained capability tokens, created and validated by the browser according to a developer-defined policy.
...
Against the natural feature of capabilities being delegatable, we need to bind them to the app identity to prevent malicious third-party libraries from using capabilities from other apps that were granted more privileges, thus circumventing the mitigation.
...
The wildcard tokens can be divided into three subtypes: Classic Wildcard Capabilities, Predefined Capabilities and Ambient Capabilities.
...
The ambent capability represents the browser's default behavior -- storing all cookies in the shared global jar -- that is received when no explicit policy is provided by the developer.
...
When constructing requests, the browser attaches the cookies encapsulated in final capabilities to the regular cookies from the global jar, ensuring that each request accurately reflects both app-specific and shared state according to the developer policy.
