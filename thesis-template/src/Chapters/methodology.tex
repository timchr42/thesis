...
The core idea is to encapsulate cookie access within fine-grained capability tokens, created and validated by the browser according to a developer-defined policy. Depending on the policy configuration, cookies are stored either in a shared global jar or in app-specific storage, analogous to the Partitioned attribute in CHIPS \cite{googlechips}.
...
We do not want the local tokens be shared between apps, as we want to scope them to the app only.
This contradicts to the natural feature of capabilities being delegatable.
...
Similar to JWTs, our tokens are signed to identify tampering attemtps.
...
In conclusion, our capability tokens are identity-bound and cryptographically protected data structures that encapsulate cookie information and metadata, enabling fine-grained access control between Android apps and the browser.
