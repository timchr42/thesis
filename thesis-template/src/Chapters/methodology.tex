...
\begin{enumerate}[label=B\arabic*)]
  ...
  \item \textbf{Backwards Compatibility}: When the developer does not provide the framework with a policy, it will default to allowing any webserver using the shared browser state, in result behaving like current cookie management. \change{is this corrent?} \unsure{ "cookie management" correct word?}
    This is done by also allowing wildcards for the domain name in addition to the cookie name and value.
    Ofcourse, for the cookie to be stored in the shared cookie jar, the global jar flag must also be set by default. \unsure{rethink this concept again! Do i even this? fin what other sense could backwards comapatibility? exist}
