...
%First, automatic policy generation based on static or dynamic analysis of app behavior could assist developers in identifying third-party domains and automatically classifying them as trusted or untrusted.
%Second, the framework could be extended to protect other forms of browser storage such as \texttt{localStorage} or \texttt{IndexedDB}, applying the same capability-based principle.
%Third, deeper browser integration, for instance through upstreaming into GeckoView or AndroidX Browser, would allow widespread adoption and consistent behavior across apps.
%
%Finally, user-facing transparency features could be explored, such as allowing users to inspect or override app policies, giving them explicit control over how their cookies are shared between applications.
