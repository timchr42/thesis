...
Before deving into the design and implementation of our capability-based mitigation against HyTrack, we first cover Capabilities and Capability-based Security (\autoref{sec:capabilities}), as well as provide a detailed overview of Custom Tabs and Trusted Web Activities on Android (\autoref{sec:customtabs-twas}).
Finally, we revisit the HyTrack attack, its weaknesses, goals, and proposed mitigations (\autoref{sec:hytrack}), and outline our threat model in (\autoref{sec:threat_model}).
...
A capability system enforces the principle of least privilege by ensuring that possession of a capability is both necessary and sufficient for performing an operation on the referenced object.
...
\begin{tablenotes}
\footnotesize
\item This table is taken from Wessel et al.'s HyTrack paper~\cite{USENIX:Wessels:2025}.
\end{tablenotes}
\end{threeparttable}
\caption{Overview of Web Content Integration Mechanisms on Android.}
...
\begin{figure}[h]
  ...
  \begin{tablenotes}
  \footnotesize
  \item This table is adapted from Wessel et al.'s HyTrack paper~\cite{USENIX:Wessels:2025}.
  \end{tablenotes}
  \caption{High-level Overview of the HyTrack Attack Flow.}
...
\label{sec:hytrack-our-approach}
TODO?
...
We want to prevent this library from tracking the app's user across multiple apps and empower app developer to use any third-party library without risking user privacy in regards to cross-app tracking via HyTrack.
...
As we hook our defense in the Androidx browser library ~\cite{android:androidxbrowser}, any developer that wants to use the malicous tracking library or any other library that relies on CTs or TWAs automatically uses our framework if they configure and provide a policy file.
