...
In this case, their apps (e.g., Instagram) opened localhost ports to receive browser tracking data -- such as mobile browsing sessions and web cookies -- sent from websites embedding Meta Pixel or Yandex scripts.
...
This can be seen by taking a closer look at the two mitigation strategies discussed by the authors of HyTrack, namely Browser State Partitioning and Forced User Interaction. 

% ---- discuss works that use Browser State Partitioning and Forced User Interaction ----
In practice, modern browsers have increasingly adoppted state partitioning to combat third-party tracking.
...
% --Start--
Google is actively working on a similar mechanism under the name CHIPS (Cookies fhaving Independent Partitioned State)~\cite{googlechips}.
...
However, CHIPS is not applicable to Android's embedded web contets like CTs or TWAs, as the top-level site is the tracker itself.
...
Other strategies, such as Limiting CTs and TWAs to First-Party Domains or introducing browser options to disable CTs and TWAs for certain domains ultimately reflect the strategies mentioned above by varying browser state partitioning and and forced user interaction, respectively.
...
By wrapping Cookies into fine-grained capability tokens created by the browser according to the app developers policy, the browser decides which cookies are allowed to be stored in the browser's (by default shared) cookie jar and which are stored in the app-specific storage, and thus not accessible to third-party libraries -- depending on a flag contained in the capability, similar to CHIPS' additional cookie attribute.
...
Wrapping the Cookies information into capability tokens is inspired by JSON Web Tokens (JWTs)~\cite{jwt}, which are widely used in web authentication to encode claims about a user or a session in a secure, verifiable manner.
...
This frees the server from having to maintain session state, as the token itself carries all the information needed andcan verify via the signature that the token has not been tampered with.
For this purpose, JWTs consist of three parts separated by dots: a header that specifies the token type and algorithm for encoding and decoding it, a payload for the actual data, and a signature of the first two parts after base64 encoding that ensures the integrity of the token.
Our approach extends this idea with communication between the browser and the app, where the browser issues capability tokens according to the app's policy and the app presenting these tokens in subsequent requests to access the browser's shared cookie jar, or to store cookies in its own app-local storage.
...
Specifically, they show that WebViews and hybrid frameworks often bypass or misapply the Same-Origin Policy (SOP), enabling attackers to inject or reuse authentication tkens across apps and domains.
