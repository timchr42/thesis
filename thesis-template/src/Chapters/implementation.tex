...
\paragraph{Modifying Custom Tab Launch Internals.}
\label{par:modifying_ct_launch}
... Approach forcing modification how CTs are launched (force session like TWA does it natively so that caller identity is known securely).
...
We therefore include a field \texttt{creator\_uid} in the intent class, which the \texttt{ActivityManagerService} ~\cite{android:activitymanagerservice} populates when processing the intent and hands it over to the requested browser.
...
Optimally, Android would natively expose the caller identity in the Intent class -- as demonstrated with our SDK --, allowing all apps to benefit from this security enhancement without requiring custom SDKs.
