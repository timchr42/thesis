...
This chapter describes the implementation of our proof-of-concept prototype and details the individual components that together realize the Byetrack mitigation outlined in the previous chapter.
...
It is expressed as a structured JSON object divided into two top-level sections: predefined and wildcard (\autoref{fig:policy_schema}).

Each section further distinguishes between two isolation scopes:
\begin{enumerate}
  \item \textbf{Global:} Cookies associated with these capabilities are stored in the browser's default cookie jar and are therefore shared across all apps using CTs/TWAs.
  \item \textbf{Private:} Cookies associated with these capabilities are kept in the app-local cookie jar and are therefore not shared with other apps.
\end{enumerate}

The predefined section specifies explicit capability bindings between domains and the cookies that are allowed to be associated with them.
...
The wildcard section defines simplified or implicit rules for domains where explicit cookie level definitions are not necessary.
...
The wildcard and predefined entries operate independently, thus a domain can appear in both lists if necessary.
...
Furthermore, the distinction between global and private in the predefined section allows a domain to hold both a global token and a private token.
This allows flexible, layered control over cookie behavior.
An example policy with explanation can be found in the appendix \autoref{appendix:policy}.
...
The token is represented as a JSON object~\cite{android:jsonobject}, as Firefox already provides helper utility for JSON parsing and serialization in the C++ layer to minimize implementation effort.
...
\begin{itemize}
  ...
  \item \texttt{app\_version}: The version name of the application, used for version consistency checks.
...
Because the \texttt{assets} directory is read-only at runtime, each APK must first be copied into the installer's private file storage before it can be installed.
...
This intent is launched through a custom \texttt{ActivityResultLauncher} ~\cite{android:activityresultlauncher}, which encapsulates Android's modern
...
For this to work, the package names of the apps to be installed must be registered in the Android Manifest~\cite{android:manifest} under the \texttt{queries} element~\cite{android:manifest.queries}. 

For inter-process communication, we employ a \texttt{ContentProvider}~\cite{android:contentprovider} exposed by the browser.
...
Thus, the content provider offered the most reliable and maintainable IPC solution for pushing the application's policy file to the browser.

\section{Browser (Fenix)}

We selected Mozilla Firefox for Android (Fenix)~\cite{firefox:fenix} as the browser base due to its open architecture and direct access to the GeckoView~\cite{firefox:geckoview} engine.
Its modifications fall into two main categories:
(1) token generation and transmitting in \texttt{GeckoView}, Mozilla's WebView like Java library for Android that exposes the \texttt{Gecko} engine's functionality, and
...
Although their responsibilities overlap conceptually, especially regarding cryptographic operations on the tokens, direct invocation across these layers proved impractical, despite the possibility for \texttt{GeckoView} code to communicate between Java and C++ via the Java Native Interface (JNI)~\cite{firefox:jni} (\autoref{subsec:jni_bridging}).
...
This step ensures that conflicting or overlapping entries are resolved according to the principle of least privilege (PoLP): private entries always take precedence over their global counterpart.
...
\paragraph{Predefined Conflict Detection.} The first downgrade check handles domain-level and cookie-level conflicts within predefined entries.
If a domain appears in both predefined global and predefined private for the same cookie name, the global cookie is removed, keeping only the private one.
...
\paragraph{Wildcard Conflict Detection.} A similar check is applied to wildcard entries.
If the same domain appears in both wildcard global and wildcard private, the global entry is discarded.

\paragraph{Independence Between Predefined and Wildcard Sections.} 
...
These functions iterate over the filtered domain and cookie lists, creating one encrypted capability token per entry using \texttt{generateSingleToken(domain, cookie\_name, "*", global\_jar, package\_name, version\_name, rights)}.
...
Finally, the encoded string is signed using HMAC-SHA256 and the signature attached to the token object separated with a dot, similar to JWTs~\cite{rfc7519}, before encrypting it using AES-CBC with a random IV using the browsers secret key.
The signature makes the tokens tamper-evident, while encryption ensures confidentiality of the token contents.

Once generated, tokens are sent to the app in a map from domain to the String representation of the JSON array via the same content provider that received the policy so that the app can persist them locally (\autoref{sec:byetracklib}).
...
\subsubsection{Threading of Capability Data}
...
Since Fenix currently does not support TWAs, any incoming TWA intent is downgraded to a standard custom tab intent and hence handled by the same processor.

Analogous to the existing \texttt{getAdditionalHeaders()} function that is used to retrieve custom HTTP headers to CT or TWA requests, we introduce a dedicated function that collects and returns all Byetrack-specific context data.
...
We need the flag to distinguish between normal website launches initiated by the user and launches initiated by an app via CTs or TWAs, as only the latter should enforce Byetrack logic.
...
\texttt{GeckoEngineSession} acts as a bridge between Android Components and GeckoView, Mozilla's Android library that exposes the Gecko browser engine APIs.
...
Here, similar to other loaders that handle fields such as \texttt{headerFilter}, \texttt{additionalHeaders}, or other \texttt{flags}, we introduced a new loader to transmit the capability tokens and UID.

Inside this loader, the \texttt{PackageManager} is used to derive the calling app's package name and version name from the UID passed in the intent.
...
The bundle is stored in the \texttt{GeckoSession} and attached to the \texttt{LoadUri} dispatch message.
When this message is processed, the loader extracts the previously stored Byetrack fields and forwards them to the browser's \texttt{fixupAndLoadURIString} function.
...
Gecko's \texttt{DocumentLoadListener} is responsible for managing the lifecycle of document loads and connecting network responses with them, and therefore the ideal place to give the threaded opaque data meaning by parsing them back into usable tokens.
...
The first step is carried out by the function \texttt{ProcessTokenBlob}, which transforms an incoming serialized token blob into validated \texttt{ByetrackToken} objects.
...
We implement a \texttt{ApplyByetrackFromLoadStateToBrowserContext} function that transfers our tokens from the \texttt{DocShellLoadState} into the active top level browsing context, where they become accessible throughout the browsing sessions lifecycle.
...
\subsubsection{Enforcement of Cookie Isolation}
%\paragraph{Stage 2 – Enforcement of Cookie Isolation.}
The actual enforcement of cookie isolation based on the capability tokens occurs in \texttt{Gecko}'s networking component \texttt{Necko}~\cite{firefox:networking-wiki, firefox:networking-docs}, especially in its \texttt{CookieService} and \texttt{HttpBaseChannel}.
When a network request is initiated, the \texttt{HttpBaseChannel} uses the \texttt{CookieService} to prepare the cookie header for outgoing requests and process incoming Set-Cookie headers from server responses and storing the cookies in the browser storage.
...
The main flow for attaching cookies to outgoing requests occurs in \texttt{HttpBaseChannel}'s \texttt{\justify AddCookiesToRequest()}.
Here, the \texttt{CookieService}'s  \texttt{GetCookieStringFromHttp} function is called to retrieve the appropriate cookies for the target domain.
...
Incoming \texttt{Set-Cookie} headers are handled by the \texttt{SetCookieHeaders} method of \texttt{HttpBaseChannel}, which processes server responses and stores cookies in the browser's cookie jar.
In the original flow, this method iteratively invokes \texttt{SetCookieStringFromHttp} from \texttt{CookieService} for each cookie string. At this point, we intercept the flow and, using the top-level browsing context, retrieve the relevant wildcard capability tokens (including the ``\texttt{enforcement}'' flag), which we pass as an additional parameter to the cookie-handling logic.

This interception point is also where CHIPS~\cite{googlechips} is implemented, making integration straightforward: CHIPS processing happens first, yielding a cookie annotated with its storage partition.
We then feed this annotated cookie into Byetrack's decision-making logic.

The extracted information is passed to \texttt{DecideCookieAction()}, which constitutes the core of our evaluation logic.
Before evaluating any token, the function inspects the ``\texttt{enforcement}'' flag to determine whether the cookie originated from a Custom Tab launch or from a normal browser-initiated navigation.
If enforcement is disabled, the function immediately returns a decision to store the cookie normally, preserving the browser's default behavior.
...
\begin{enumerate}
  \item \textbf{Predefined tokens} always take priority over wildcard tokens.
  \item \textbf{Private tokens} override global ones within the same category, ensuring stricter privacy when applicable.
  \item \textbf{Wildcard tokens} apply only when no predefined token matches and define the handling of all cookies for the corresponding domain.
\end{enumerate}

The result is encapsulated in a \texttt{ByetrackCookieDecision} object, which specifies both the chosen action (store, capture, or reject) and the token that authorized it.
Based on this decision, the integration proceeds as follows:

\begin{enumerate}
  \item \textbf{StoreNormally:}
  If the cookie is permitted for global storage (via a predefined or wildcard global token), Byetrack allows the native Gecko insertion flow (\texttt{storage->AddCookie()}) to proceed.
  This preserves expected behavior for legitimate cases such as session cookies or user preferences.

  \item \textbf{CapturePredefined:}
  For cookies explicitly declared as private in the policy, the cookie value is embedded into the corresponding token, converting it into a final token.
  The token's access rights are elevated to \texttt{READ\_WRITE}, enabling app-side access through Byetrack's utility interface (\autoref{sec:utility}).
  Instead of being stored in the global jar, this updated token is forwarded to \texttt{StageTokenForReturn()}, which serializes and queues it for return to the originating app.

  \item \textbf{CaptureWildcard:}
  For wildcard rules designating a domain as private, both the cookie name and value are captured into the token.
  Unlike predefined captures, the token's access rights remain at \texttt{NONE} to prevent a malicious library from simply reading the value and exfiltrating it, which would defeat isolation.

  \item \textbf{Reject:}
  If no token matches the cookie or if the policy explicitly forbids handling cookies for the domain, the cookie is dropped, preventing unauthorized cross-site tracking.
\end{enumerate}
...
To synchronize the browser's enforcement results with the application process, we extend \texttt{Gecko}'s networking stack with a dedicated emission helper implemented in \texttt{HttpBaseChannel}'s \texttt{EmitByetrackTokensToGeckoView()}.
This function is invoked from \texttt{HttpChannelParent}'s \texttt{OnStopRequest()}, that is, at the exact moment when an HTTP request completes and all cookies have been processed by the \texttt{CookieService}.
Note that at this point, our subsystem has already collected any filled in tokens into the temporary map stored in the channel object by the \texttt{StageTokenForReturn()} function.

The \texttt{EmitByetrackTokensToGeckoView()} helper serializes this in-memory map into a compact JSON structure and forwards it through \texttt{Gecko}'s observer service.
...
Using Mozilla's JSON writer utility, the function iterates over the map entries, associating each domain with an array of token strings.
...
By performing this emission inside \texttt{HttpChannelParent}'s \texttt{OnStopRequest()}, we guarantee that the final set of captured tokens is only emitted once the HTTP transaction has completed and all cookies have been processed.
This ensures that no partial or intermediate state is sent to the embedding application.

On the embedding side, the \texttt{byetrack-final-tokens} observer event is handled within \texttt{GeckoViewNavigation}, the same place where the threading in \texttt{GeckoView} started.
The observer's \texttt{observe} method processes the serialized JSON map and uses \texttt{GeckoView}'s event dispatch system to send a message of type \texttt{GeckoView:Byetrack:FinalTokens} carrying the tokens and the application identity (package name).
This event is caught on the Java side inside the responsible \texttt{GeckoSession}, the same location where \texttt{LoadUri} and similar events are processed.
When the \texttt{GeckoView:Byetrack:FinalTokens} event is received, the Java handler constructs a \texttt{ContentValues}~\cite{android:contentvalues} object containing the token JSON and writes it to the application's registered \texttt{ContentProvider}~\cite{android:contentprovider}, allowing it to update its local capability store.
...
In this \texttt{ContentProvider}, we leverage the parameter \texttt{method} of the \texttt{call} function~\cite{android:contentprovider:call} to distinguish between the different utility functions.
...
The data for each function to work on is passed via remaining parameters of the function (\texttt{arg} and \texttt{extras}) if necessary. 

\paragraph{GetTokenNames.} 
If the method parameter is set to \texttt{get\_token\_cookie\_names}, the function expects a list of capability tokens in JSON array format as the \texttt{arg} parameter.
...
If the method parameter is set to \texttt{get\_token\_cookie\_value}, the function expects a single encrypted capability token as the \texttt{arg} parameter.
...
The provider returns the cookie value only if the payload's is readable (\texttt{canRead() is true}), in which case the value is included in the ``\texttt{value}'' field of the result \texttt{Bundle}.
...
If write access is permitted, the new cookie value is taken from the \texttt{extras} bundle under the key ``\texttt{value}''.
...
This provider only implements the \texttt{insert()}~\cite{android:contentprovider:insert} method, as neither querying nor deletion is required.
When the browser calls \texttt{insert()}, the library first verifies the calling package name to ensure that only legitimate browsers can write to the app's token store.
...
This is handled by the \texttt{ByetrackClient} via its \texttt{attachTokens()} and \texttt{injectTokens()} methods.
...
Developers can use the same convenient methods \texttt{launchUrl()}~\cite{android:customtabsintent:launchurl} and \texttt{launchTrustedWebActivity()}~\cite{android:trustedwebactivityintent:launchtrustedwebactivity} for launching a custom tabs intent and trusted web activity intent as before; the library automatically enriches them with the necessary Byetrack metadata prior to launch.
...
Specifically, both \texttt{CustomTabsIntent}'s \texttt{launchUrl()} and \texttt{TrustedWebActivityIntent}'s \texttt{launchTrustedWebActivity()} were extended to call the \texttt{attachTokens()} method of the exposed client before invoking the activity.
\autoref{code:customtablaunch} shows this concretely for the \texttt{launchUrl()} function; the TWA launch function follows the same pattern.
...
Both HyTrack demo applications, \texttt{CrossAppLauncher} and \texttt{CrossAppTrackerOne}, originally launched TWA) using Chrome's \texttt{android-browser-helper} library~\cite{chrome:androidbrowserhelper}, which automatically establishes a TWA connection to Chrome for convenience.
...
Although Firefox does not natively support TWAs yet, no further changes to the applications were required: the TWA intents are transparently downgraded to standard CT intents within the browser.
...
The most crucial challenge was securely identifying the application that launched a CT.  
...
In contrast, TWAs are not affected by this problem, as by design, they require a session with the browser over a Binder channel that inherently conveys the caller's identity.
...
We therefore explored multiple approaches to securely determine the calling application's identity, many of which turned out to be insecure or impractical within our threat model (\autoref{sec:threat_model}).
...
Consequently, the malicious app could launch a CT with tokens belonging to the legitimate app, thereby bypassing our protections.

\paragraph{Shared Secret and Challenge-Response.}
A more advanced approach was to establish a shared secret between the application and the browser (e.g., using the \texttt{KeyStore}~\cite{android:keystore}) and perform a challenge–response protocol during CT launch.  
...
However, deriving the identity from such a medium and linking it securely to the CT launch intent proved difficult, as any identifier accessible to the app is also accessible to the tracking library, and can thus be spoofed.
...
When the CT is later launched, the intent itself remains empty, and the browser uses the previously committed data.  

While this design provides strong authenticity guarantees --- the commitment occurs over a secure channel --- it introduces significant complexity.  
...
In this approach, the intent originally used to launch the CT is wrapped into a \texttt{PendingIntent} flagged as immutable, and capability tokens are attached as extras.  
The browser can then obtain the caller identity securely via \texttt{getCreatorPackage()} or \texttt{getCreatorUid()} and invoke the \texttt{PendingIntent} on behalf of the app.  
...
Another potential approach was to redesign the CT launch mechanism to more closely resemble the session-based launch flow used by TWAs.
...
Whether through a two-phase commit–reveal sequence, wrapping the launch intent in an immutable \texttt{PendingIntent}, or enforcing a \texttt{TWA}-style session-binding flow, all approaches deviate from the standard \texttt{AndroidX Browser} contract.
...
This implies that legacy apps that would now use our Byetrack-enabled browser library could no longer open CTs with a standard, non Byetrack-aware browser, limiting usability and adoption.
...
To maintain compatibility with existing apps, we ultimately opted for a solution that allows the app to launch CTs using the standard API while still securely conveying the caller's identity.
...
Here, the \texttt{realCallingUid} is injected into the intent immediately before the system hands control to the target application (e.g., the browser).
...
This approach provides a trustworthy, unspoofable channel for conveying the caller's identity to the receiving browser, enabling capability-based cookie isolation without breaking compatibility with existing app behavior.
However, this solution requires applications to compile against our customized Android SDK, which may limit adoption in practice. 
...
% ---------------------------------------
%Und was du hier auch noch diskutieren könntest wäre ein Option wo du Apps zwingst ihre Identity zu sharen. Also der Browser bearbeitet den Request nur wenn
%https://developer.android.com/reference/android/app/ActivityOptions#setShareIdentityEnabled(boolean)
%
%der Setter gecalled wurde. Das würde ähnlich zur Injection der Tokens funktionieren und könnte auch ohne OS modification funktionieren. Bin ich aber selber erst gestern Abend drüber gestolpert, sonst wäre das einen Versuch wert gewesen.
\paragraph{Android Identity Sharing Mechanism.}
Another possible direction would be to rely on Android's built-in identity-sharing mechanism, which allows an activity to learn the identity of its caller when launching a new activity.
This can be enabled by setting \texttt{ActivityOptions.setShareIdentityEnabled(boolean)} to true~\cite{android:setshareidentityenabled}.

However, adopting this mechanism would require every app that launches a CT to explicitly enable identity sharing and for the browser to reject any launch request that does not set this flag.
This would break compatibility with existing apps, similar to the other approaches discussed above.
If Android were to enable this flag by default for all CT launches, no additional SDK modifications would be necessary, as the underlying functionality already exists.
...
However, these calls require an active \texttt{GeckoRuntime} instance~\cite{firefox:jni:runtime}.
