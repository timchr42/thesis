...
\begin{itemize}
  \item 
    \textbf{Custom Installer Application:} Responsible for installing test apps and transmitting their declared policies to the browser.

\item
  \textbf{Byetrack Helper Library:} A reusable client-side library that manages capability tokens, communication with the browser, and transparent token injection into outgoing intents.

\item
\textbf{Modified AndroidX Browser Library:} A drop-in replacement for the standard AndroidX Browser module that automatically integrates Byetrack logic into every CT and TWA invocation.
...
Both layers communicate through structured Java Native Interface (JNI) ~\cite{firefox:jni} bindings and share a common cryptographic key for token signing and verification.
...
Although their responsibilities overlap conceptually --- especially cryptographic operations on the tokens ---, direct invocation across these layers proved impractical, despite the possibility for \texttt{GeckoView} code to communicate between Java and C++ via the JNI (see \autoref{subsec:jni_bridging}).
...
These functions iterate over the filtered domain and cookie lists, creating one encrypted capability token per entry using \texttt{generateSingleToken(domain, cookie\_name, ""*", globalJar, package\_name, version\_name, rights)}.
...
Analogous to the existing \texttt{getAdditionalHeaders()} function --- used to retrieve custom HTTP headers to CT or TWA requests ---, we introduce a dedicated function that collects and returns all Byetrack-specific context data.
...
The first step is carried out by the function \textsc{ProcessTokenBlob}, which transforms an incoming serialized token blob into validated \texttt{ByetrackToken} objects.
...
We implement a \texttt{ApplyByetrackFromLoadStateToBrowserContext} function that transfers our tokens from the \texttt{DocShellLoadState} into the active top level browsing context, where they become accessible trhoughout the browsing sessions lifecycle.
...
For incoming Set-Cookie headers, the \texttt{SetCookieHeaders} of \texttt{HttpBaseChannel} processes and stores cookies received from server responses.
It calls \texttt{SetCookieStringFromHttp} defined in \texttt{CookieService} iteratively for each cookie string and stores them in the browser's cookie jar.
...
The extracted information is then passed to the function \texttt{DecideCookieAction()}, which encapsulates the core of our decision-making logic.
...
\begin{enumerate}
  \item 
    \textbf{StoreNormally:}
    If the cookie is authorized for global storage (e.g., by a predefined or wildcard global token), Byetrack simply continues the native Gecko cookie insertion flow conducted by \texttt{storage->AddCookie()}.
...
For cookies explicitly defined in the policy as private (e.g., session identifiers that must not leak cross-site), the cookie's value is embedded into the associated token by updating the token's value field, essentially turning it into a final token.
In addition, the token's access rights are updated to \texttt{READ\_WRITE}, making the cookie name and value accessible to the application by the additional utility Byetrack provides (\autoref{sec:utility}).
Instead of being stored in the browser's global jar, this updated token is forwarded to \texttt{StageTokenForReturn()} --- a helper routine that serializes the token information and stages it for return to the application the CT launch originated from.
...
Note, that for simplicity we only consider the standard cookie representation here and do not check for additional attributes such as \texttt{Path}, \texttt{Domain}, or \texttt{Secure}, which could be added in a more advanced implementation \autoref{chap:future_work}.
...
To synchronize the browser's enforcement results with the application process, we extends \texttt{Gecko}'s networking stack with a dedicated emission helper implemented in \texttt{HttpBaseChannel}'s \texttt{EmitByetrackTokensToGeckoView()}.
...
Note that at this point, our subsystem has already collected any filled in tokens tokens into the temporary map stored in the channel object by the \texttt{StageTokenForReturn()} function.
...
The observer's \textsc{observe()} method processes the serialized JSON map and uses \texttt{GeckoView}'s event dispatch system to send a message of type \texttt{GeckoView:Byetrack:FinalTokens} carrying the tokens and the application identity (package name).
...
In this content provider, we leverage the parameter ``method'' of the \texttt{call} function ~\cite{android:contentprovider:call} to distinguish between the different utility functions.
...
Specifically, both \texttt{CustomTabsIntent}'s \texttt{launchUrl()} and \texttt{TrustedWebActivityIntent}'s \texttt{launchTrustedWebActivity()} were extended to call the \texttt{attachTokens()} method of the exposed client before invoking the activity (\autoref{code:customtablaunch}). 
...
A more advanced approach was to establish a shared secret between the application and the browser (e.g., using the \texttt{KeyStore} ~\cite{android:keystore}) and perform a challenge–response protocol during Custom Tab launch.  
...
The browser can then obtain the caller identity securely via \texttt{getCreatorPackage()} or \textsc{getCreatorUid()} and invoke the \texttt{PendingIntent} on behalf of the app.  
...
This implies that legacy apps that would now use our Byetrack-enabled browser library could no longer open Custom Tabs with a standard, non Byetrack-aware browser, limiting usability and adoption.
...
However, adopting this mechanism would require every app that launches a Custom Tab to explicitly enable identity sharing and for the browser to reject any launch request that does not set this flag.
This would break compatibility with existing apps, similar to the other approaches discussed above.
If Android were to enable this flag by default for all Custom Tab launches, no additional SDK modifications would be necessary, as the underlying functionality already exists.
